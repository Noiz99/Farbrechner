# ---------------- Eingabefunktion ----------------
def eingabe_genotyp():
    # Gene für Extension bis Splashed White
    ex_vater = input("Gib den Genotyp des Vaters für Extension ein: ")
    ag_vater = input("Gib den Genotyp des Vaters für Agouti ein: ")
    dun_vater = input("Gib den Genotyp des Vaters für Dun ein: ")
    cr_vater = input("Gib den Genotyp des Vaters für Cream ein: ")
    ch_vater = input("Gib den Genotyp des Vaters für Champagne ein: ")
    g_vater  = input("Gib den Genotyp des Vaters für Grey ein: ")
    kit_vater = input("Gib den Genotyp des Vaters für Kit ein: ")
    z_vater = input("Gib den Genotyp des Vaters für Silver ein: ")
    o_vater = input("Gib den Genotyp des Vaters für Overo ein: ")
    spl_vater = input("Gib den Genotyp des Vaters für Splashed ein: ")
    lp_vater = input("Gib den Genotyp des Vaters für LP ein: ")
    patn1_vater = input("Gib den Genotyp des Vaters für PATN1 ein: ")
   
    ex_mutter = input("Gib den Genotyp der Mutter für Extension ein: ")
    ag_mutter = input("Gib den Genotyp der Mutter für Agouti ein: ")
    dun_mutter = input("Gib den Genotyp der Mutter für Dun ein: ")
    cr_mutter = input("Gib den Genotyp der Mutter für Cream ein: ")
    ch_mutter = input("Gib den Genotyp der Mutter für Champagne ein: ") 
    g_mutter  = input("Gib den Genotyp der Mutter für Grey ein: ")   
    kit_mutter = input("Gib den Genotyp der Mutter für Kit ein: ")
    z_mutter = input("Gib den Genotyp der Mutter für Silver ein: ")
    o_mutter = input("Gib den Genotyp der Mutter für Overo ein: ")
    spl_mutter = input("Gib den Genotyp der Mutter für Splashed ein: ")
    lp_mutter = input("Gib den Genotyp der Mutter für LP ein: ")
    patn1_mutter = input("Gib den Genotyp der Mutter für PATN1 ein: ")

    return (ex_vater, ex_mutter, ag_vater, ag_mutter, dun_vater, dun_mutter,
...
def vererbung_dun(dun_vater, dun_mutter):
    ...
    if dun_vater == "dd" and dun_mutter == "dd":
        possible_dun_fohlen = ["dd"]
    elif dun_vater == "DD" and dun_mutter == "DD":
        possible_dun_fohlen = ["DD"]
    elif dun_vater == "Dd" and dun_mutter == "dd":
        possible_dun_fohlen = ["Dd", "dd"]
    elif dun_vater == "dD" and dun_mutter == "dd":
        possible_dun_fohlen = ["dd", "Dd"]
    elif dun_vater == "Dd" and dun_mutter == "Dd":
        possible_dun_fohlen = ["DD", "Dd", "dD", "dd"]
    elif dun_vater == "Dd" and dun_mutter == "dD":
        possible_dun_fohlen = ["DD", "Dd", "dD", "dd"]
    elif dun_vater == "DD" and dun_mutter == "Dd":
        possible_dun_fohlen = ["DD", "Dd"]
...
def vererbung_cream(cr_vater, cr_mutter):
    ...
    if cr_vater == "crcr" and cr_mutter == "crcr":
        ...
        possible_cr_fohlen = ["CrCr", "crcr", "CrCr", "crCr"]
    elif cr_vater == "Crcr" and cr_mutter == "crCr":
        possible_cr_fohlen = ["CrCr", "crcr", "CrCr", "crCr"]
    elif cr_vater == "crCr" and cr_mutter == "Crcr":
        possible_cr_fohlen = ["CrCr", "crcr", "CrCr", "crCr"]
...
def vererbung_grey(g_vater, g_mutter):
    """
    Erzeugt alle möglichen Kombinationen des Grey-Gens, 
    wobei wir direkt mit den eingegebenen 2-Zeichen-Strings arbeiten.
    """
...
def apply_grey_effect(end_colors, grey_combination):
    # Wenn alle möglichen Grey-Genotypen "gg" sind, bleibt alles unverändert:
    if all(g == "gg" for g in grey_combination):
        return end_colors
    # Wenn alle möglichen Grey-Genotypen nicht "gg" sind, überdeckt Grey alles:
    elif all(g != "gg" for g in grey_combination):
        return ["Grey"]
    else:
        # Bei einer Mischung: Für die Fälle, in denen kein Grey exprimiert wird, bleiben die Basisfarben erhalten,
        # und für die Fälle mit dominanten Allelen erscheint "Grey".
        return list(set(end_colors + ["Grey"]))

# ---------------- Gene: Kit ----------------
def vererbung_kit(kit_vater, kit_mutter):
    """
    Erwartet Eltern-Kit-Genotypen als 4-Zeichen-String, z.B. "TOTO", "SB00", "WI00", "RnRn" oder "0000".
    Liefert alle möglichen 4-Zeichen-Kit-Genotypen (ohne den letalen Fall "WIWI").
    """
...
def apply_kit_effect(end_colors, kit_combinations):
    ...
    for geno in kit_combinations:
        eff = kit_effect(geno)
        kit_effects.add(eff)  # fügt auch den leeren String hinzu, wenn kein Effekt
    result = set()
    for color in end_colors:
...
def apply_silver_effect(end_colors, silver_combination):
    # Wenn keiner der Silver-Genotypen ein Großbuchstaben "Z" enthält, also alle rezessiv sind, bleibt alles unverändert.
    if not any("Z" in genotype for genotype in silver_combination):
        return end_colors
    else:
        result = []
        for color in end_colors:
            # Chestnut wird nicht beeinflusst.
            if color.lower() == "chestnut":
                result.append(color)
            else:
                result.append("Silver " + color)
        # Zusätzlich behalten wir auch die Originalfarben bei (je nach gewünschter Logik)
...
def apply_overo_effect(end_colors, overo_combination):
    # Falls ein letaler Overo ("OO") vorkommt, überdeckt dieser alles.
    if "OO" in overo_combination:
        return ["Overo (Lethal)"]
    # Falls mindestens ein heterozygotes Overo ("Oo") vorkommt, hänge " Overo" als Suffix an alle Endfarben.
...
def vererbung_splashed_white(spl_vater, spl_mutter):
    """
    Erwartet: Beide Eltern geben ihren Splashed White Genotyp als 6-Zeichen-String ein,
    z. B. Vater: "SPLspl", Mutter: "splSPL".
    Teilt jeden String in zwei 3-Zeichen-Allele und liefert alle möglichen Kombinationen.
    """
...
def splashed_white_effect(spl_genotype):
    """
    Liefert den Effekt für Splashed White:
      - Wenn der Genotyp exakt "splspl" (alle Kleinschreibung) ist, wird kein Effekt ausgegeben.
      - Andernfalls wird "Splashed White" zurückgegeben.
    """
...
def apply_splashed_white_effect(end_colors, spl_white_combinations):
    # Wenn alle Kombinationen exakt "splspl" sind, dann keine Änderung.
    if all(geno == "splspl" for geno in spl_white_combinations):
        return end_colors
...
def apply_lp_patn1_effect(end_colors, lp_genotypes, patn1_genotypes):
    """
    Bestimmt für jede Kombination aus LP- und PATN1-Genotyp den entsprechenden Musterungseffekt und
    hängt diesen als Suffix an jede Farbe in end_colors an.
    
    Regeln:
      - Wenn LP homozygot "LPLP" ist:
          - Bei PATN1 in {"P1P1", "P1p1", "p1P1"}: Effekt = "Few Spot"
          - Andernfalls: Effekt = "Snowcap"
      - Wenn LP heterozygot ("LPlp" oder "lpLP") ist:
          - Bei PATN1 in {"P1P1", "P1p1", "p1P1"}: Effekt = "Leopard"
          - Andernfalls: Effekt = "Schabrackentiger"
      - Alle anderen LP-Genotypen (z.B. "lplp") haben keinen Effekt.
    """
    result = set()
    # Menge der PATN1-Genotypen, die den dominanten Effekt auslösen
...
def gruppiere_und_sortiere_farben(final_colors):
    ...
    reihenfolge = ["Chestnut", "Wildbay", "Bay", "Sealbrown", "Black", "Red", "Grulla"]
    # Verwende ein Tupel als Key: (Index in reihenfolge oder ein hoher Wert, Name)
    sortierte_basisfarben = sorted(gruppen.keys(), key=lambda x: (reihenfolge.index(x) if x in reihenfolge else len(reihenfolge), x))
    
    for basis in sortierte_basisfarben:
        print(f"\n{basis}:")
        for variant in sorted(gruppen[basis]):
            print(f"  {variant}")
...
def main():
    (ex_vater, ex_mutter, ag_vater, ag_mutter, dun_vater, dun_mutter,
     cr_vater, cr_mutter, ch_vater, ch_mutter, g_vater, g_mutter,
     kit_vater, kit_mutter, z_vater, z_mutter, o_vater, o_mutter,
     spl_vater, spl_mutter, lp_vater, lp_mutter, patn1_vater, patn1_mutter) = eingabe_genotyp()
    ...
    print("Mögliche Allele für Extension:", possible_ex_fohlen)
    print("Mögliche Allele für Agouti:", possible_ag_fohlen)
    print("Mögliche Allele für Dun:", possible_dun_fohlen)
    print("Mögliche Allele für Cream:", possible_cr_fohlen)
    print("Mögliche Allele für Champagne:", possible_champagne)
    print("Mögliche Allele für Grey:", possible_grey)
    print("Mögliche Allele für Kit:", possible_kit)
    print("Mögliche Allele für Silver:", possible_silver)
    print("Mögliche Allele für Overo:", possible_overo)
    print("Mögliche Allele für Splashed:", possible_splashed_white)
    ...
    print("\nDie Basisfarben des Fohlens basierend auf Extension und Agouti sind:")
    for farbe in moegliche_basisfarben:
        print(farbe)
    ...
    # LP/PATN1-Effekt anhängen
    final_with_muster = apply_lp_patn1_effect(finale_farben, vererbung_lp(lp_vater, lp_mutter), vererbung_patn1(patn1_vater, patn1_mutter))
        
    print("\nDie gruppierten und sortierten Endfarben des Fohlens sind:")
    gruppiere_und_sortiere_farben(final_with_muster)
